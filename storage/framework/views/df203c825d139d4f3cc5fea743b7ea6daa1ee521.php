...
<div id="user-statut" <?php if(Auth::check()): ?>data-statutuser="<?php echo e(auth()->user()->statut); ?>" <?php else: ?> data-statutuser="guest"<?php endif; ?>></div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        var url = '<?php echo e(route('rdvs.datatable')); ?>';
        var statutrdvC;
                    ...
                    columns: [
                        { data: '_id',"searchable": false,"orderable":false, "className":'noExl no-filter',
                            ...
                            render: function(data){
                                var url = '<?php echo e(route('rdv.edit',':_id')); ?>';
                ...
                $("#empTables").DataTable({
                    ...
                    columns: [
                        { data: '_id',"searchable": false,"orderable":false, "className":'noExl no-filter',
                            name: '_id',
                            render: function(data){
                                var url = '<?php echo e(route('rdvs.details',':_id')); ?>';
                                url = url.replace(':_id',data);
...
<script>
  $(function () {
    ...
    $('#calendar').fullCalendar({
		...
		<?php endif; ?>
		<?php if(isset($calendarclient)): ?>
        events: [
				<?php $__currentLoopData = $calendarclient; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            {
                id : '<?php echo e($event['googleuid']); ?>',
                client_priv : '<?php echo e($client->prenom.' '.$client->nom); ?>',
                adresse_details :'<?php echo $event['adresse_details']?$event['adresse_details']:''; ?>',
                note_details : '<?php echo $event['note_details']?$event['note_details']:''; ?>',
				<?php if(isset(auth()->user()->statut)): ?>
						<?php if(isset(auth()->user()->statut)): ?>
						<?php if((auth()->user()->statut == 'Superviseur') || (auth()->user()->statut == 'Agent')): ?>
                title : 'Créneau horaire indisponible',
				...
				<?php else: ?>
                title : '<?php echo $event['title']; ?>',
                titre_details : '<?php echo $event['titleDetails']; ?>',
				<?php endif; ?>
						<?php endif; ?>
						<?php endif; ?>
                ...
                borderColor : '<?php echo e($event['borderColor']); ?>',
            },
			<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ],
		<?php endif; ?>
  ...
  $('.filteragenda').on('change',function(){
	if ($(this).val()=='all'){
		var url = '<?php echo e(route("agendas.index")); ?>';
			window.location.href = url;
	} else {
		let edit_id = $(this).children(':selected').data('idclient');
		var url ='<?php echo e(route("agendas.get_agenda_client", ":id")); ?>';
  ...
  $(document).ready(function() {
		...
		<?php if(isset($dataevent)): ?>
			<?php $__currentLoopData = $dataevent; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
			{
				...
				// backgroundColor : '<?php echo $event->borderColor; ?>',
				borderColor : '#f39c12',
				//borderColor : '<?php echo $event->borderColor; ?>',
			},
			<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<script>
  ...
  $(document).ready(function() {
		...
		events: [
		<?php if(isset($dataevent3)): ?>
			<?php $__currentLoopData = $dataevent3; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event3): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
			{
				title : '<?php echo $event3->title; ?>', 
				start : '<?php echo $event3->start; ?>',
				// end: '<?php echo $event3->start; ?>',
				backgroundColor : '<?php echo $event3->backgroundColor; ?>',
				borderColor : '<?php echo $event3->borderColor; ?>',
			},
			<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
		<?php endif; ?>
		],
...
<script>
  ...
  $(document).ready(function() {
		...
		forceEventDuration: true,
		events: [
			<?php if(isset($dataeventclient)): ?>
				<?php $__currentLoopData = $dataeventclient; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
				{
					title : 'Indisponible',
					start : '<?php echo e($event['start']); ?>',
					 end: '<?php echo e($event['end']); ?>',
...
<!-- UPLOAD image-->
	<script src="<?php echo e(asset ('/js/upload_image.js')); ?>"></script>
<!-- UPLOAD AUDIO -->
<script>
...
<script>
	...
	<?php /**PATH E:\projet freelance\crm1\resources\views/layouts/partials/scripts.blade.php ENDPATH**/ ?>