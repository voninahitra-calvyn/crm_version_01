    ...
    <div class="box box-danger">
        <form class="form-horizontal" method="post" action="{{ route('rdvs.update', $rdv->id) }}" enctype="multipart/form-data">
            ...
            <div class="box-body">
				...
				<input type="text" class="form-control hidden" value="{{$clientComResp->prenom}}" name="client_prenompriv" id="client_prenompriv_camp">
				...
				<div class="form-group">
						<label for="typerdv" class="col-sm-2 control-label">Client : {{--$rdv->typerdv--}}</label>
						<div class="col-sm-10">
							<select class="form-control select2" name="cli" id="campagne-client" style="width: 100%;" @if (Auth::user()->statut <> 'Administrateur' && Auth::user()->statut <> 'Staff') disabled @endif>
									<option value="{{$rdv->cli}}" selected
											data-campagne-client-nom="{{$clientComResp->nom}}"
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <select class="form-control select2" name="typerdv" id="typerdv" style="width: 100%;" @if (Auth::user()->statut <> 'Administrateur' && Auth::user()->statut <> 'Staff') disabled @endif>
				...
				<div class="form-group"  id="groupcentreappel">
						<label for="typerdv" class="col-sm-2 control-label">Centre d'appels : {{--$rdv->typerdv--}}</label>
						<div class="col-sm-10">
							<select class="form-control select2" name="centreappel_societe" id="centreappel_societe_ch" style="width: 100%;" @if (Auth::user()->statut <> 'Administrateur' && Auth::user()->statut <> 'Staff') disabled @endif>
								<option value="Groupe Administration" {{$rdv->centreappel_societe =="Groupe Administration" ? 'selected' : ''}} data-centre="{{$rdv->id_groupe}}">
									Groupe Administration </option>
								@foreach($centreAll as $centre)
									<option value="{{$centre->societe}}" {{$centre->societe==$rdv->centreappel_societe ? 'selected' : ''}} data-centre="{{$centre->_id}}">{{$centre->societe}}</option>
								@endforeach
							</select>
						</div>
				</div>
				...
				<div class="form-group"  id="groupresponsableagent">
						<label for="typerdv" class="col-sm-2 control-label">Responsable/agent : {{--$rdv->typerdv--}}</label>
						<div class="col-sm-10">
							<select class="form-control select2" name="responsableagent" id="centreappel_societe_agent" style="width: 100%;" @if (Auth::user()->statut <> 'Administrateur' && Auth::user()->statut <> 'Staff') disabled @endif>
								<option value="{{$rdv->user_id}},{{$rdv->user_prenom}} {{$rdv->user_nom}}" {{--{{$rdv->responsableagent==$rdv->responsableagent ? 'selected' : ''}}--}} selected>{{$rdv->user_prenom}} {{$rdv->user_nom}}</option>
							</select>
						</div>
				</div>
{{--
				...
				@if (Auth::user()->statut != 'Responsable' || Auth::user()->statut != 'Commercial')
							...
							<div class="form-group"  id="created_at">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" name="#" id="#" placeholder="Date de création" value="{{ $rdv->created_at->format('j-m-Y H:i')}}" disabled>
								</div>
							</div>
				@endif			
			...
			@if (Auth::user()->statut != 'Responsable' || Auth::user()->statut != 'Commercial')	
				<div class="form-group" id="updated_at">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" name="#" id="#" placeholder="Date de modification" value="{{ $rdv->updated_at->format('j-m-Y H:i')}}" disabled>
								</div>
				</div>
			@endif	
			<div class="form-group @if (Auth::user()->statut <> 'Administrateur' && Auth::user()->statut <> 'Staff') hidden @endif" id="ismailEnvoi" >
