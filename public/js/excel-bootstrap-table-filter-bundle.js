...
var FilterMenu = function () {
    ...
    FilterMenu.prototype.searchdateToggle = function (date1) {
        ...
        this.inputs.filter(function (input) {
			...
			var dateListeTab = new Date(input.value.toLowerCase().split("-").reverse().join("/"));
		// $('#datedeb_filtre').val(dateListeTab);
			...
			return inputDatedebut <= dateListeTab;
				...
				// $('#datedeb_filtre').val('hhhhhhh');
				// $('#datedeb_filtre').val(date1.split("-").reverse().join("-"));
    ...
    FilterCollection.prototype.updateRowVisibility = function (filterMenus, rows, ths) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            for (var j = 0; j < selectedLists.length; j++) {
				...
				// alert('selectedLists['+j+']: '+selectedLists[j]);
				$(document).ready(function() {
					// $('#idrdv_filtre').val(selectedLists[j]);
				});
				...
				if(selectedLists[j].column==2){
					contenu +='/';
				}
				if((selectedLists[j].column!=0) && (selectedLists[j].column!=1)){
					contenu +=content+';';
                    // alert('content['+selectedLists[j].column+']'+content);
					// $(document).ready(function() {
						// $('#idrdv_filtre').val(content);
					// });
				}
                $(rows[i]).show();
				var tds2 = rows[i].children;
            }
        }
					$(document).ready(function() {
						$('#idrdv_filtre').val(contenu);
					});
    };
