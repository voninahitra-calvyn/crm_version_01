							...
							<select class="form-control select2 filteragenda" name="clientrdv" id="clientrdv" style="width: 100%;" >
								...
								@foreach($comptesclient as $cli)
									@if (Auth::user()->statut == 'Superviseur' || Auth::user()->statut == 'Agent')
										<option value="{{$cli->prenom}} {{$cli->nom}}">{{$cli->nom}}</option>
									@else
										<option value="{{$cli->prenom}} {{$cli->nom}}">{{$cli->prenom}} {{$cli->nom}}</option>
									@endif
								@endforeach
        ...
        <div class="box box-danger">
            ...
            <div class="box-footer no-padding">
				@if (Auth::user()->statut == 'Administrateur' || Auth::user()->statut == 'Staff')
					<!-- <b>Lien partageable public :</b> <a href="{{ route('agendas.agenda2', Auth::user()->_id) }}" target="_blank" >{{ url('/admnstaff')}}/{{Auth::user()->_id}}</a> -->
				@elseif (Auth::user()->statut == 'Responsable' || Auth::user()->statut == 'Commercial')
					<!-- <b>Lien pour ajouter ces rendez-vous à votre agenda :</b> <a href="{{ route('agendas.agenda3', Auth::user()->_id) }}" target="_blank" >{{ url('/comresp')}}/{{Auth::user()->_id}}</a> -->
					<!-- <b>Lien partageable public :</b> <a href="{{ route('agendas.comresp_public', Auth::user()->_id) }}" target="_blank" >{{ url('/comresp_public')}}/{{Auth::user()->_id}}</a> -->
				@endif
			</div>
			...
			<div class="modal-content">
				<form class="form-horizontal" method="POST" action="{{ route('agendas.store') }}">
					...
					<div class="modal-body">
						...
						@else
						  <input type="text" class="form-control hidden" name="searchTerm" id="searchTerm">
						  <input type="text" value="{{Auth::user()->prenom}} {{Auth::user()->nom}}" class="form-control hidden" name="client_priv" id="client_priv">
						@endif
